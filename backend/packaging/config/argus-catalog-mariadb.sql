...
CREATE TABLE IF NOT EXISTS federation_instances (
    id INT AUTO_INCREMENT PRIMARY KEY COMMENT 'PK',
    instance_key VARCHAR(64) NOT NULL UNIQUE COMMENT '허브 내 peer 식별 키 (URN namespace prefix)',
    name VARCHAR(200) NOT NULL COMMENT '표시용 이름',
    base_url VARCHAR(500) NOT NULL COMMENT 'peer base URL (예: https://catalog.team-a.internal)',
    auth_token TEXT COMMENT 'peer 호출용 서비스 토큰(선택). Phase 1 에서 암호화 전환 예정',
    mode VARCHAR(20) NOT NULL DEFAULT 'LIVE' COMMENT '연합 모드 (HARVEST/LIVE/HYBRID)',
    sync_interval_sec INT NOT NULL DEFAULT 900 COMMENT 'HARVEST 동기화 주기(초) — 후속 단계 사용',
    status VARCHAR(20) NOT NULL DEFAULT 'ACTIVE' COMMENT '상태 (ACTIVE=연합검색 포함 / PAUSED=제외)',
    display_fields TEXT COMMENT '소비자(허브) 표시 선택 — 화면에 표시할 capability 키 JSON 배열(NULL=전부)',
    description TEXT COMMENT '설명',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '생성 시각',
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '수정 시각'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='연합(federation) 대상 peer Argus Catalog 인스턴스 레지스트리';

-- 기존 배포(컬럼이 없던 DB)도 멱등하게 수렴
ALTER TABLE federation_instances ADD COLUMN IF NOT EXISTS display_fields TEXT
    COMMENT '소비자(허브) 표시 선택 — 화면에 표시할 capability 키 JSON 배열(NULL=전부)';


-- ===========================================================================
-- Federation HARVEST 미러 — peer 메타데이터 로컬 복제 + 허브 모델 재임베딩
--   임베딩 컬럼은 pgvector 미지원으로 TEXT fallback (시맨틱 검색은 PostgreSQL 한정).
-- ===========================================================================

CREATE TABLE IF NOT EXISTS federation_datasets (
    id INT AUTO_INCREMENT PRIMARY KEY COMMENT 'PK',
    instance_id INT NOT NULL COMMENT 'peer 인스턴스 federation_instances(id) 참조, CASCADE' REFERENCES federation_instances(id) ON DELETE CASCADE,
    remote_urn VARCHAR(500) NOT NULL COMMENT 'peer 내부 URN',
    federated_urn VARCHAR(600) NOT NULL UNIQUE COMMENT '네임스페이스 URN ({instance_key}::{remote_urn})',
    name VARCHAR(255) NOT NULL COMMENT '데이터셋 물리명(예: sakila.actor)',
    display_name VARCHAR(255) COMMENT '데이터셋 논리명(예: 배우) — 없으면 NULL',
    datasource_name VARCHAR(200) COMMENT '데이터소스 이름',
    datasource_type VARCHAR(100) COMMENT '데이터소스 유형',
    summary VARCHAR(200) COMMENT '한 줄 요약',
    description TEXT COMMENT '설명',
    qualified_name VARCHAR(500) COMMENT '정규화 이름',
    origin VARCHAR(50) COMMENT '환경(DEV/STAGING/PROD)',
    field_count INT NOT NULL DEFAULT 0 COMMENT 'peer 측 스키마 컬럼 개수(가져오기 시점)',
    has_sample BOOLEAN NOT NULL DEFAULT false COMMENT '미러 샘플 데이터 보유 여부(federation/samples)',
    source_text TEXT COMMENT '재임베딩 변경 감지용 원본 텍스트',
    remote_created_at TIMESTAMP NULL COMMENT 'peer 측 등록 시각(원본 created_at)',
    remote_updated_at TIMESTAMP NULL COMMENT 'peer 측 최종 수정 시각(증분 동기화 watermark)',
    harvested_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '가져온 시각',
    UNIQUE (instance_id, remote_urn),
    INDEX ix_federation_datasets_instance (instance_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='HARVEST 로 가져온 peer 데이터셋 로컬 미러 (read-only)';
-- 기존 배포(컬럼이 없던 DB)도 멱등하게 수렴
ALTER TABLE federation_datasets ADD COLUMN IF NOT EXISTS display_name VARCHAR(255) COMMENT '데이터셋 논리명(예: 배우)';
ALTER TABLE federation_datasets ADD COLUMN IF NOT EXISTS field_count INT NOT NULL DEFAULT 0 COMMENT 'peer 측 스키마 컬럼 개수(가져오기 시점)';
ALTER TABLE federation_datasets ADD COLUMN IF NOT EXISTS has_sample BOOLEAN NOT NULL DEFAULT false COMMENT '미러 샘플 데이터 보유 여부(federation/samples)';
ALTER TABLE federation_datasets ADD COLUMN IF NOT EXISTS remote_created_at TIMESTAMP NULL COMMENT 'peer 측 등록 시각(원본 created_at)';

CREATE TABLE IF NOT EXISTS federation_dataset_embeddings (
    id INT AUTO_INCREMENT PRIMARY KEY COMMENT 'PK',
    federation_dataset_id INT NOT NULL UNIQUE COMMENT '미러 데이터셋 federation_datasets(id) 참조, CASCADE' REFERENCES federation_datasets(id) ON DELETE CASCADE,
    embedding TEXT NOT NULL COMMENT '임베딩 벡터 (pgvector, 384차원 — MariaDB 는 TEXT fallback)',
    source_text TEXT NOT NULL COMMENT '임베딩 원본 텍스트',
    model_name VARCHAR(200) NOT NULL COMMENT '임베딩 모델명',
    provider VARCHAR(50) NOT NULL COMMENT '임베딩 제공자',
    dimension INT NOT NULL DEFAULT 384 COMMENT '벡터 차원',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '생성 시각',
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '수정 시각'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='HARVEST 미러 데이터셋의 허브 모델 재임베딩';
...
CREATE TABLE IF NOT EXISTS federation_sync_runs (
    ...
    datasets_total INT NOT NULL DEFAULT 0 COMMENT 'peer 가 보고한 전체 데이터셋 수(진행률 표시용)',
    datasets_seen INT NOT NULL DEFAULT 0 COMMENT '가져올 때 관측한 데이터셋 수',
    datasets_upserted INT NOT NULL DEFAULT 0 COMMENT 'upsert 한 데이터셋 수',
    datasets_embedded INT NOT NULL DEFAULT 0 COMMENT '재임베딩한 데이터셋 수',
    datasets_pruned INT NOT NULL DEFAULT 0 COMMENT '정리(prune)한 데이터셋 수',
    phase VARCHAR(20) NOT NULL DEFAULT 'FETCH' COMMENT '현재 단계 (FETCH/EMBED/FINALIZE)',
    phase_done INT NOT NULL DEFAULT 0 COMMENT '현재 단계 진척 분자',
    phase_total INT NOT NULL DEFAULT 0 COMMENT '현재 단계 진척 분모',
    error TEXT COMMENT '실패 사유',
    started_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '시작 시각',
    finished_at TIMESTAMP NULL COMMENT '종료 시각',
    INDEX ix_federation_sync_runs_instance (instance_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='페더레이션 HARVEST 실행 이력 (관측성)';

-- 기존 배포(컬럼이 없던 DB)도 멱등하게 수렴
ALTER TABLE federation_sync_runs ADD COLUMN IF NOT EXISTS datasets_total INT NOT NULL DEFAULT 0
    COMMENT 'peer 가 보고한 전체 데이터셋 수(진행률 표시용)';
ALTER TABLE federation_sync_runs ADD COLUMN IF NOT EXISTS phase VARCHAR(20) NOT NULL DEFAULT 'FETCH'
    COMMENT '현재 단계 (FETCH/EMBED/FINALIZE)';
...
CREATE TABLE IF NOT EXISTS federation_lineage (
    id INT AUTO_INCREMENT PRIMARY KEY COMMENT 'PK',
    instance_id INT NOT NULL COMMENT '엣지를 보고한 peer federation_instances(id) 참조, CASCADE' REFERENCES federation_instances(id) ON DELETE CASCADE,
    source_urn VARCHAR(500) NOT NULL COMMENT '원본 데이터셋 URN(peer 내부)',
    target_urn VARCHAR(500) NOT NULL COMMENT '대상 데이터셋 URN(peer 내부)',
    relation_type VARCHAR(32) NOT NULL DEFAULT 'READ_WRITE' COMMENT '관계 유형',
    lineage_source VARCHAR(32) NOT NULL DEFAULT 'QUERY_AGGREGATED' COMMENT '리니지 출처',
    description TEXT COMMENT '설명',
    harvested_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '가져온 시각',
    UNIQUE (instance_id, source_urn, target_urn, relation_type),
    INDEX ix_federation_lineage_instance (instance_id),
    INDEX ix_federation_lineage_source (source_urn),
    INDEX ix_federation_lineage_target (target_urn)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='HARVEST 로 가져온 peer 리니지 엣지(URN→URN, cross-instance stitching)';
