...
직접 읽어 프로파일링·검증을 수행하고, 결과를 다시 API 로 반입(import)한다.

서버 내장 검증과의 차이:
  - 프로파일 "스냅숏"이 아닌 원본 전체 데이터로 평가하므로
    ACCEPTED_VALUES / REGEX 처럼 서버에서 profile-only 로 제한되던 규칙도
    실제 데이터 기준으로 정확히 평가한다.
  - 대용량 테이블은 Spark 클러스터 자원으로 분산 처리할 수 있다.
...
원본 DB 접속 정보:
  - 기본: 카탈로그의 데이터 소스 연결 설정(GET /catalog/datasources/{id}/configuration)
    에서 host/port/database/username/password 를 읽어 JDBC URL 을 조립한다.
  - 카탈로그에 연결 설정이 없으면 --jdbc-url / --jdbc-user / --jdbc-password 로
    직접 지정한다.

흐름:
    로그인 → URN 으로 데이터셋 조회 → 규칙 조회 → JDBC 로 원본 읽기
    → 프로파일 계산 → 규칙 평가 → 프로파일/결과 반입 → 요약 출력
"""
...
# ---------------------------------------------------------------------------
# 카탈로그 API 클라이언트 (표준 라이브러리만 사용 — 추가 의존성 없음)
# ---------------------------------------------------------------------------
...
class CatalogApi:
    ...
    def get_datasource_configuration(self, datasource_id: str) -> dict | None:
        ...
        try:
            ...
            return None  # 연결 설정 미등록 — JDBC 인자로 폴백
...
# ---------------------------------------------------------------------------
# JDBC 접속 — 카탈로그 연결 설정 또는 CLI 인자에서 조립
# ---------------------------------------------------------------------------

JDBC_TEMPLATES = {
    # MySQL 프로토콜 계열 — StarRocks 는 MySQL 와이어 프로토콜(9030) 호환
    "mysql": ("jdbc:mysql://{host}:{port}/{database}", "com.mysql.cj.jdbc.Driver", 3306),
    "mariadb": ("jdbc:mysql://{host}:{port}/{database}", "com.mysql.cj.jdbc.Driver", 3306),
    "starrocks": ("jdbc:mysql://{host}:{port}/{database}", "com.mysql.cj.jdbc.Driver", 9030),
    # PostgreSQL 프로토콜 계열
    "postgresql": ("jdbc:postgresql://{host}:{port}/{database}", "org.postgresql.Driver", 5432),
    "greenplum": ("jdbc:postgresql://{host}:{port}/{database}", "org.postgresql.Driver", 5432),
    "redshift": ("jdbc:redshift://{host}:{port}/{database}",
                 "com.amazon.redshift.jdbc42.Driver", 5439),
    "oracle": ("jdbc:oracle:thin:@//{host}:{port}/{database}", "oracle.jdbc.OracleDriver", 1521),
    "tibero": ("jdbc:tibero:thin:@{host}:{port}:{database}", "com.tmax.tibero.jdbc.TbDriver", 8629),
    "mssql": ("jdbc:sqlserver://{host}:{port};databaseName={database};encrypt=false",
              "com.microsoft.sqlserver.jdbc.SQLServerDriver", 1433),
    "sqlserver": ("jdbc:sqlserver://{host}:{port};databaseName={database};encrypt=false",
                  "com.microsoft.sqlserver.jdbc.SQLServerDriver", 1433),
    # SQL 엔진 — database 자리에 카탈로그/DB 명
    "trino": ("jdbc:trino://{host}:{port}/{database}", "io.trino.jdbc.TrinoDriver", 8080),
    "hive": ("jdbc:hive2://{host}:{port}/{database}", "org.apache.hive.jdbc.HiveDriver", 10000),
    "impala": ("jdbc:impala://{host}:{port}/{database}", "com.cloudera.impala.jdbc.Driver", 21050),
    ...
    "snowflake": ("jdbc:snowflake://{host}.snowflakecomputing.com/?db={database}",
                  "net.snowflake.client.jdbc.SnowflakeDriver", 443),
}
...
def resolve_jdbc(
    ...
    """JDBC (url, driver, user, password) 결정. CLI 인자 > 카탈로그 연결 설정."""
    template, driver, default_port = JDBC_TEMPLATES.get(
        ds_type, (f"jdbc:{ds_type}://{{host}}:{{port}}/{{database}}", "<DRIVER>", 0),
    )
    driver = args.jdbc_driver or driver

    if args.jdbc_url:
        if not (args.jdbc_user and args.jdbc_password is not None):
            raise SystemExit("--jdbc-url 사용 시 --jdbc-user / --jdbc-password 도 필요합니다.")
        return args.jdbc_url, driver, args.jdbc_user, args.jdbc_password

    cfg = api.get_datasource_configuration(datasource_id) or {}
    if not cfg.get("host"):
        raise SystemExit(
            "카탈로그에 데이터 소스 연결 설정이 없습니다. "
            "--jdbc-url/--jdbc-user/--jdbc-password 인자로 직접 지정하세요."
        )
    database = cfg.get("database") or default_database
    url = template.format(host=cfg["host"], port=int(cfg.get("port", default_port)), database=database)
    return url, driver, cfg.get("username", ""), cfg.get("password", "")
...
def compute_profile(df: DataFrame, row_count: int) -> list[dict]:
    """컬럼별 NULL/고유값/최소·최대/평균 통계를 한 번의 집계로 계산."""
    ...
    for f in df.schema.fields:
        ...
        aggs.append(F.countDistinct(F.col(c)).alias(f"{c}__uniq"))
        # 바이너리 컬럼(BLOB 등) — min/max 가 무의미하고 거대하므로 생략
        if not isinstance(f.dataType, BinaryType):
            aggs.append(F.min(F.col(c)).cast("string").alias(f"{c}__min"))
            aggs.append(F.max(F.col(c)).cast("string").alias(f"{c}__max"))
    ...
    for f in df.schema.fields:
        ...
        uniq = int(row.get(f"{c}__uniq") or 0)
        mean = row.get(f"{c}__mean")

        # 최빈값 top 5 — 저카디널리티(코드성) 컬럼만 추가 집계
        top_values = None
        if 0 < uniq <= 20 and not isinstance(f.dataType, BinaryType):
            tv = (df.filter(F.col(c).isNotNull()).groupBy(c).count()
                  .orderBy(F.desc("count")).limit(5).collect())
            top_values = [{"value": str(r2[0]), "count": int(r2[1])} for r2 in tv]

        profiles.append({
            "column_name": c,
            "column_type": f.dataType.simpleString().upper(),
...
# ---------------------------------------------------------------------------
# 규칙 평가 — 서버 _evaluate_rule 과 동일한 시맨틱.
# 단, ACCEPTED_VALUES / REGEX 는 원본 전체 데이터로 실제 평가한다.
# ---------------------------------------------------------------------------
...
def _spark_samples(sub: "DataFrame", limit: int = 5) -> list[dict] | None:
    """위반 행을 카탈로그 반입용 샘플(dict 목록)로 변환 — 최대 limit 행, 값은 문자열화."""
    rows = sub.limit(limit).collect()
    if not rows:
        return None
    return [
        {k: (None if v is None else str(v)) for k, v in row.asDict().items()}
        for row in rows
    ]


def collect_failed_samples(rule: dict, df: "DataFrame") -> list[dict] | None:
    ...
    try:
        if check == "NOT_NULL":
            return _spark_samples(df.filter(c.isNull()))
        if check == "UNIQUE":
            dup_vals = [r[0] for r in (
                df.groupBy(column).count().filter("count > 1").limit(5).collect()
            )]
            if not dup_vals:
                return None
            return _spark_samples(df.filter(c.isin(dup_vals)))
        if check == "ACCEPTED_VALUES" and expected:
            allowed = [v.strip() for v in expected.split(",")]
            return _spark_samples(df.filter(c.isNotNull() & ~c.cast("string").isin(allowed)))
        if check == "REGEX" and expected:
            return _spark_samples(df.filter(c.isNotNull() & ~c.cast("string").rlike(expected)))
        if check == "MIN_VALUE" and expected is not None:
            return _spark_samples(df.filter(c.cast("double") < float(expected)))
...
def evaluate_rule(rule: dict, df: DataFrame, row_count: int,
                  col_map: dict[str, dict],
                  spark: SparkSession | None = None,
                  jdbc: tuple[str, str, str, str] | None = None,
                  checks_dir: str | None = None) -> tuple[bool, str, str]:
    ...
    if check == "ACCEPTED_VALUES":
        ...
        bad = df.filter(F.col(column).isNotNull() &
                        ~F.col(column).cast("string").isin(accepted)).count()
    ...
    if check == "REGEX":
        ...
        bad = df.filter(F.col(column).isNotNull() &
                        ~F.col(column).cast("string").rlike(expected)).count()
        return bad == 0, f"위반 {bad}행", \
            f"정규식 '{expected}' 기준 위반 {bad}행 (전체 데이터 검사)"

    if check == "CUSTOM_SQL":
        # 기대값의 SELECT 쿼리를 JDBC pushdown 으로 원본에서 실행.
        # 규약: 첫 행 첫 컬럼 = 위반 건수 (0 = 통과). read-only 계정 사용 전제.
        if not expected:
            return False, "N/A", "expected_value 에 검증 SQL 이 필요합니다"
        reason = validate_custom_sql(expected)
        if reason:
            return False, "N/A", f"SQL 검증 실패: {reason}"
        if spark is None or jdbc is None:
            return False, "N/A", "원본 JDBC 컨텍스트가 없어 커스텀 SQL 을 실행할 수 없습니다"
        url, driver, user, password = jdbc
        try:
            result = (
                spark.read.format("jdbc")
                .option("url", url)
                .option("driver", driver)
                .option("dbtable", f"({expected.strip().rstrip(';')}) AS custom_check")
                .option("user", user)
        ...
        try:
            ...
            return False, str(result[0][0]), "커스텀 SQL 의 첫 컬럼이 숫자(위반 건수)가 아닙니다"
...
def process_dataset(
    spark: SparkSession, api: CatalogApi, dataset_id: int, table: str,
    jdbc: tuple[str, str, str, str], dry_run: bool, checks_dir: str,
) -> dict:
    ...
    try:
        ...
        passed_count = 0
        for rule in rules:
            passed, actual, detail = evaluate_rule(rule, df, row_count, col_map, spark=spark, jdbc=jdbc, checks_dir=checks_dir)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="카탈로그 데이터셋 품질 배치 (PySpark 프로파일링 + 규칙 검증)",
    ...
    # 원본 JDBC — 생략 시 카탈로그의 데이터 소스 연결 설정 사용
    parser.add_argument("--jdbc-url", default=None, help="원본 JDBC URL (카탈로그 설정 대신 직접 지정)")
    parser.add_argument("--jdbc-user", default=None, help="원본 DB 계정")
    parser.add_argument("--jdbc-password", default=None, help="원본 DB 비밀번호")
    parser.add_argument("--jdbc-driver", default=None, help="JDBC 드라이버 클래스 (기본: 타입별 표준)")
    ...
    # 1) 대상 데이터셋 결정 — (id, table 이름) 목록과 데이터 소스 정보
    if args.urn:
    ...
    # 2) JDBC 접속 결정 (같은 데이터 소스이므로 1회)
    jdbc = resolve_jdbc(args, api, ds_type, datasource_id, targets[0][1].split(".")[0])

    spark = (
        SparkSession.builder
        .appName(f"dataset-quality-{datasource_id}")
        .getOrCreate()
    )
    ...
    for dataset_id, table in targets:
        try:
            summaries.append(process_dataset(spark, api, dataset_id, table, jdbc, args.dry_run, checks_dir))
        except SystemExit:
            raise
